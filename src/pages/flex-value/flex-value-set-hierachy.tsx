...
export default function FlexValueSetHierachyPage() {
  ...
  */
  ...
  function onDeleteFlexValueSet(flexValueSetId: number|undefined): void {
  ...
  const LinkCodeCell = ({ item }: { item: FlexValueSet }) => {
    return (
      <div>


        <Link to={`/flex-value-set/edit?id=${item.flexValueSetId}`} style={{ textDecoration: "none" }}>
          <div style={{ color: "#1976d2", cursor: "pointer" }}>
            {item.flexValueSetCode}
          </div>
        </Link>
    ...
    <Paper>
      ...
      <Toolbar disableGutters>
        <Button variant="contained" color="primary" onClick={() => {
          ...
          navigate("/flex-value-set/add"); // 🔥 redirect
        }
        ...
        <Table>
          ...
          <TableBody>
            {dataRepo?.data?.map((item) => (
              <TableRow key={String(item.flexValueSetCode || "")}>
                <TableCell>
                  <LinkCodeCell item={item}></LinkCodeCell>
                </TableCell>
                ...
                <TableCell>
                  ...
                  <Group>
                    ...
                    onClick={() => { 
                      navigate(`/flex-value-set/edit?id=${item.flexValueSetId}`);
                        // getForEdit(item.flexValueSetId);
                      }}
                      >
                      {MdEdit({ size: 18 })}
                    </ActionIcon>
