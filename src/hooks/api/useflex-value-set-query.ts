...
import { GetByIdWithDetailApi, GetFlexValueSet, GetFlexValueSetByIdApi } from "../../services/flex-value-set.service";
...
export const useGetByIdWithDetail = (
  ...
  const query = useQuery<FlexValueSet>({
    queryKey: ["flexValueSetid", flexValueSetId],
    queryFn: () => GetByIdWithDetailApi(flexValueSetId),
    enabled: !!flexValueSetId, // ❗ chỉ gọi khi có id, tránh lỗi khi id chưa được set
    // enabled: false, // ❗ không auto call

 
  staleTime: 5 * 60 * 1000,   // 🔥 không refetch liên tục
  refetchOnWindowFocus: false // 🔥 tránh auto call
...
export const useFlexValueSetGetChildLinkQuery = (
